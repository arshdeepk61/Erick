...
1. Start the app: `mvn spring-boot:run` (listens on `http://localhost:8080`, context path `/api`).
2. In Bruno: **Open Collection** → select this `api-tests` folder.
3. Pick the **Local** environment (top-right). It defines `baseUrl=http://localhost:8080/api`.
